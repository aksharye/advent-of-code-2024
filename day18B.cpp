...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        bytes.pb({x,y});
        grid[x][y] = '#'; // initially place all bytes
    }

    // iterate backwards through number of bytes placed
    for (ll b = bytes.size()-1; b >= 0; b--) {
        ...
        // if steps isn't -1, exit is reachable, and the previous byte is the byte we want
        if (steps != -1) {
            cout << bytes[b+1].first << "," << bytes[b+1].second << endl;
            break;
        }

        // remove byte
        grid[bytes[b].first][bytes[b].second] = '.';
    }
}